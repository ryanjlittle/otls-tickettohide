...
class ProofProverMsgData(spec._StructBase):
    ...
    def replace(self, proof:bytes|None=None) -> Self:
        return type(self)((self.proof if proof is None else B16Raw.create(proof)))

    @classmethod
    def create(cls,proof:bytes) -> Self:
        return cls(proof=B16Raw.create(proof))
...
class B16ProofProverMsgData(BoundedProofProverMsgData):
    _LENGTH_TYPES = (Uint16, )

class ProofProverMsg(spec._SpecificSelectee[ProverMsgTypes, B16ProofProverMsgData]):
    _SELECT_TYPE = ProverMsgType
    _DATA_TYPE = B16ProofProverMsgData
    _SELECTOR = ProverMsgTypes.PROOF

    @classmethod
    def create(cls, proof:bytes) -> Self:
        return cls(data=B16ProofProverMsgData.create(proof))
...
@dataclass(frozen=True)
class KeyShareTest1ProverMsgData(spec._StructBase):
    _member_names: ClassVar[tuple[str,...]] = ('share',)
    _member_types: ClassVar[tuple[type[Spec],...]] = (tls13_spec.KeyShareEntry,)
    share: tls13_spec.KeyShareEntry

    def replace(self, share:tls13_spec.KeyShareEntry|None=None) -> Self:
        return type(self)((self.share if share is None else share))

    @classmethod
    def create(cls,share:tls13_spec.KeyShareEntry) -> Self:
        return cls(share=share)

    def uncreate(self) -> tls13_spec.KeyShareEntry:
        return (self.share)

class BoundedKeyShareTest1ProverMsgData(KeyShareTest1ProverMsgData, Spec):
    _LENGTH_TYPES: tuple[type[spec._Integral],...]
...
class B16KeyShareTest1ProverMsgData(BoundedKeyShareTest1ProverMsgData):
    _LENGTH_TYPES = (Uint16, )

class KeyShareTest1ProverMsg(spec._SpecificSelectee[ProverMsgTypes, B16KeyShareTest1ProverMsgData]):
    _SELECT_TYPE = ProverMsgType
    _DATA_TYPE = B16KeyShareTest1ProverMsgData
    _SELECTOR = ProverMsgTypes.KEY_SHARE_TEST1

    @classmethod
    def create(cls, share:tls13_spec.KeyShareEntry) -> Self:
        return cls(data=B16KeyShareTest1ProverMsgData.create(share))
...
@dataclass(frozen=True)
class KeyShareTest2ProverMsgData(spec._StructBase):
    _member_names: ClassVar[tuple[str,...]] = ('group','pubkey',)
    _member_types: ClassVar[tuple[type[Spec],...]] = (tls13_spec.NamedGroup,B16Raw,)
    group: tls13_spec.NamedGroup
    pubkey: B16Raw
...
class BoundedKeyShareTest2ProverMsgData(KeyShareTest2ProverMsgData, Spec):
    _LENGTH_TYPES: tuple[type[spec._Integral],...]
...
class B16KeyShareTest2ProverMsgData(BoundedKeyShareTest2ProverMsgData):
    _LENGTH_TYPES = (Uint16, )

class KeyShareTest2ProverMsg(spec._SpecificSelectee[ProverMsgTypes, B16KeyShareTest2ProverMsgData]):
    _SELECT_TYPE = ProverMsgType
    _DATA_TYPE = B16KeyShareTest2ProverMsgData
    _SELECTOR = ProverMsgTypes.KEY_SHARE_TEST2

    @classmethod
    def create(cls, group:tls13_spec.NamedGroup, pubkey:bytes) -> Self:
        return cls(data=B16KeyShareTest2ProverMsgData.create(group, pubkey))
